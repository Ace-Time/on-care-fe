        ...
        >
          ...
          <div class="item-right">
            <div class="score-chips">
              <span class="score-badge total">총 {{ alt.totalScore }}점</span>
              <span class="score-badge distance">거리 {{ alt.distanceScore }}점</span>
              <span class="score-badge tag">태그 {{ alt.tagScore }}점</span>
            </div>
...
<script setup>
   ...
   * - 페이지 객체: res.data.data / list / content 중 하나 + total/totalElements 존재
   */
  const pickPage = (res) => {
    const d = res?.data
    if (Array.isArray(d)) {
  ...
  /**
   *  리스트 정규화(점수 필드 포함)
   * - 백엔드 필드명이 totalScore/distanceScore/tagScore이면 그대로
   * - 혹시 score/distScore/tagsScore 형태로 내려와도 대응
   */
  ...
  /**
   * 핵심 로드:
   * - CONFIRMED: uiPage(1-based) -> server page(0-based) (서버 size=3)
  ...
  const load = async () => {
    ...
    try {
      ...
      if (isConfirmed) {
        ...
        pagedAlternatives.value = normalizeList(list) // 점수 포함해서 정규화
      ...
      const normalized = normalizeList(list) // 점수 포함해서 정규화
      pagedAlternatives.value = normalized.slice(
        serverOffsetInPage,
        serverOffsetInPage + UI_PAGE_SIZE
      )
    } catch (e) {
...
.score-badge.total { background: #ede9fe; color: #7c3aed; }
.score-badge.distance { background: #e0f2fe; color: #0284c7; }
.score-badge.tag { background: #dcfce7; color: #15803d; }
.name {
  font-size: 14px;
  font-weight: 700;
  color: #111827;
