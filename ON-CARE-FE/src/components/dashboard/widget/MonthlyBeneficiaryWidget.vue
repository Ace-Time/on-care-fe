...
const props = defineProps({
  title: { type: String, default: '월별 신규/탈퇴 수급자 추이' },
...
const chartSeries = computed(() => {
  ...
  return [
    {
      name: '신규 수급자',
      data: sortedData.value.map(item => item.newBeneficiaryCount)
    },
    {
      name: '탈퇴 수급자',
      data: sortedData.value.map(item => item.withdrawnBeneficiaryCount)
    }
  ];
});
...
const chartOptions = computed(() => ({
  ...
  yaxis: {
    labels: {
      formatter: (value) => value.toFixed(0) // 정수 표기
    }
  },
  // 색상 (파란색: 신규, 붉은색: 탈퇴)
